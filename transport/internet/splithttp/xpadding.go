...
// variants kept per (method, length). 4 is enough to make identical-length
// padding look different across requests without blowing memory.
const strictSamplesPerLength = 4
...
func AdaptivePaddingRange(baseFrom, baseTo int32, payloadSize int) (from, to int32) {
	...
	// Soften bucket boundaries with a bounded jitter factor derived from the
	// payload size. Adjacent payload sizes land in slightly different
	// sub-ranges, breaking the deterministic "payload bucket ↔ padding length"
	// mapping that a passive observer could exploit.
	jitter := int32(payloadSize % 7) // 0..6
