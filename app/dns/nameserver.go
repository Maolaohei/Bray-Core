...
func NewServer(ctx context.Context, dest net.Destination, dispatcher routing.Dispatcher, disableCache bool, serveStale bool, serveExpiredTTL uint32, clientIP net.IP) (Server, error) {
	if address := dest.Address; address.Family().IsDomain() {
		...
		case strings.EqualFold(u.Scheme, "quic+local"): // DNS-over-QUIC Local mode
			// WARNING: quic+local dials the QUIC server directly (quic.DialAddr),
			// bypassing the routing/proxy chain. If the DoQ server IP is blocked
			// by the GFW, this scheme is unusable; prefer https:// (remote DoH)
			// for censored networks. A remote quic:// mode is not implemented.
